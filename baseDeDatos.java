...
public class baseDeDatos {
	...
	public void funcion2()
	{
		...
		GraphDatabaseService db= dbFactory.newEmbeddedDatabase(Directorio);
	      ...
	      ExecutionResult execResult = execEngine.execute("MATCH (x:PERSONA)-[rel:NUM_CORREOS]->(y:PERSONA)\n"+ "WHERE rel.Correos >= 6\n"+ "RETURN rel, x, y");
	...
	public void funcion5()
	{
		...
		GraphDatabaseService db= dbFactory.newEmbeddedDatabase(Directorio);
		  ...
		  ExecutionEngine execEngine2 = new ExecutionEngine(db);
	      ExecutionResult execResult2 = execEngine.execute("MATCH (x:PERSONA)-[rel:NUM_CORREOS]->(y:PERSONA)\n"+"WHERE rel.Correos >= 20\n"+ "RETURN  x");
	      String results = execResult2.dumpToString();
	      System.out.println("LAs personas que más enviaron correos fueron"+results);
	      ExecutionResult execResult2 = execEngine2.execute("MATCH (x:PERSONA)-[rel:NUM_CORREOS]->(y:PERSONA)\n"+"WHERE rel.Correos <= 4\n"+ "RETURN  x");
	      String results2 = execResult2.dumpToString();
	      System.out.println("LAs personas que menos enviaron correos fueron"+results2);
	}
	
	public void funcion6(String nodo1,String nodo2)
	{
		File Directorio=new File("baseDeDatos");
		GraphDatabaseFactory dbFactory = new GraphDatabaseFactory();
		GraphDatabaseService db= dbFactory.newEmbeddedDatabase(Directorio);
		ExecutionEngine execEngine3 = new ExecutionEngine(db);
	    ExecutionResult execResult3 = execEngine3.execute("MATCH (x:PER"+nodo1+")-[rel:NUM_CORREOS]->(y:PER"+nodo2+")\n"+"WHERE rel.Correos < 5\n"+ "RETURN  rel");
	    String results3 = execResult3.dumpToString();
	}
